<template>
  <div>
    <div class="card mb-3" v-for="(data, index) in props.datas.data" :key="index">
      <div class="d-flex p-2 px-3">
        <div class="d-flex flex-row align-items-center">
          <img :src="computedUserAvatar(data)" height="50" width="50" />
      ...
      <div v-if="$route.path == '/dashboard'">
        <div v-if="data.get_post_attachment_images.length == 1">
          <div class="w-100" v-for="(file, index) in data.get_post_attachment_images" :key="index">
        ...
        <div class="d-inline-flex w-100" v-if="data.get_post_attachment_images.length == 2">
          <div class="w-50 bg-dark" v-for="(file, index) in data.get_post_attachment_images" :key="index">
            <router-link :to="`/post/${data.id}?item=${index}`">
              <img
                v-if="file.file_type == 'png' || file.file_type == 'jpg' || !file.file_type"
                :src="file.image_url"
        ...
        <div v-if="data.get_post_attachment_images.length == 4">
          <div class="row mx-0">
            <div class="col-md-6 px-0" v-for="(file, index) in data.get_post_attachment_images" :key="index">
              <div class="bg-dark h-100">
                <router-link :to="`/post/${data.id}?item=${index}`">
                  <img
                    v-if="file.file_type == 'png' || file.file_type == 'jpg' || !file.file_type"
      ...
      <div class="card px-4 rounded-1 py-1 mx-3 mb-2" v-for="file in data.get_post_attachment_files" :key="file.id">
        <div class="d-flex flex-row justify-content-between">
          <div class="text-primary">
            {{ file.name }}
          </div>
          <div class="text-primary">
            <a role="button" @click="download(file)"><i class="fa fa-download"></i></a>
          </div>
        </div>
      </div>
        ...
        <div class="d-flex">
          ...
          <div class="ms-auto">
            <div class="dropdown dropdown-menu-end">
              <a href="" class="dropdown-toggle" data-bs-toggle="dropdown">Sort comments</a>
              <ul class="dropdown-menu">
                <li>
                  <a class="dropdown-item" @click="latestComments(data)" role="button">Latest Comments</a>
                </li>
                <li>
                  <a class="dropdown-item" @click="oldComments(data)" role="button">Old Comments</a>
                </li>
              </ul>
            </div>
          </div>
...
const displayPostImage = (img) => {
  return `${import.meta.env.VITE_API_URL}/storage/posts/files/${img}`;
};
...
const computedUserAvatar = (data) => {
  if (data.get_user_details.profile_img) {
    return import.meta.env.VITE_API_URL + data.get_user_details.image_url;
  } else {
