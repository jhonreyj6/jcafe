<template>
  <div>
    <div class="card shadow-0 border rounded-3 mb-3" v-for="(data, index) in props.datas.data" :key="index">
      <div class="card-body">
        <div class="row">
          ...
          <div class="col-md-6 col-7">
            ...
            <h6>Size:</h6>
            <div class="mt-2">
              <span v-for="(size, index) in data.get_variants" :key="index">
                <input v-if="index == 0" type="hidden" :id="`input_size_` + data.id" :value="data.default_variant" />
                <button
                  class="btn btn-sm me-1 text-white"
                  :class="data.default_variant == size.value ? 'btn-info' : 'btn-outline-info'"
          ...
          <div class="col-md-3 col-5 border-sm-start-none">
            ...
            <div class="text-center">
              ...
              <router-link v-else to="/login" class="btn btn-outline-primary mb-2 my-auto"> Add to cart </router-link>

              <div class="text-muted">{{ data.default_stocks }} stocks left</div>
            </div>
          </div>
        </div>
      </div>
