    ...
    <div class="space-intro">
      <div class="container">
        <div class="mb-5">
            ...
            <div class="col-lg-8">
              <div class="card mb-4">
                <div class="card-body p-4">
                  <div class="h5" v-if="cart_items">Shopping Cart</div>
                  <div v-else class="h5">No items in the cart yet.</div>
                  <hr />

                  <div class="d-flex flex-row justify-content-between align-items-center mb-4">
                    <div class="form-check" v-if="cart_items">
                      <label class="form-check-label fs-5">
                        <input class="form-check-input" ref="select_all" type="checkbox" @change="selectAll($event)" />
                        Select All
                      </label>
                    </div>
                    <div v-if="cart_items">
                      <a role="button" class="text-danger fs-5" @click="deleteCartItem">
                  ...
                  <div class="card mb-3" v-for="(item, index) in cart_items" :key="index">
                    <div class="card-body">
                      <div class="row">
                        <div class="col-md-2 mb-2">
                          <label :for="'label_' + item.id">
                            <img :src="item.image_url" :alt="item.name" class="img" height="150" />
                          </label>
                        ...
                        <div class="col-md-7 mb-2">
                          ...
                          <div class="text-black-50 mb-1">
                            Variant:
                            {{ item.product_variant_details.value + item.product_variant_details.unit }}
                          </div>
                          <div class="product-description">
                            {{ item.product_details.description }}
                          </div>
                          <div class="mt-2">
                            <Star :rating="item.product_details.rating" />
                          </div>
                        </div>
                        <div class="col-md-3">
              ...
              <div class="card">
                <div class="card-body p-4">
                  ...
                  <div class="mt-4">
                    ...
                    <div class="text-center">
                      <button @click="stripe.state = true && orders.length" class="btn btn-primary w-100">
                        Checkout
                      </button>
                    </div>
                  </div>
          ...
          <div v-show="stripe.state && orders.length" class="card p-4 mx-auto stripe-form">
            <div class="mb-2">
              <div class="d-flex flex-row justify-content-between align-items-center mb-4">
                <h4>Payment Details</h4>
                <button type="button" class="btn btn-danger text-lg" @click="stripe.state = false">Go back</button>
              </div>
...
const computedSubTotal = computed(() => {
  ...
  orders.value.forEach((elem, index) => {
    cart_items.value.forEach((item) => {
      if (elem == item.id) {
        subtotal.value = subtotal.value += item.product_variant_details.price * item.quantity;
      }
    });
  });
...
const changeQuantity = (item) => {
  ...
  if (document.getElementById("input_" + item.id).value > item.product_variant_details.stock) {
    document.getElementById("input_" + item.id).value = item.product_variant_details.stock;
  }

  // commit mutation for cart count
  if (item.quantity > +document.getElementById("input_" + item.id).value) {
    userStore().$patch((state) => {
      state.cart_count - (item.quantity - +document.getElementById("input_" + item.id).value);
    });
  } else {
    userStore().$patch((state) => {
      state.cart_count + document.getElementById("input_" + item.id).value - item.quantity;
    });
  }
...
const processPayment = async (e) => {
  ...
  const { paymentMethod, error } = await stripe.value.load_stripe.createPaymentMethod(
    "card",
...
onMounted(async () => {
  ...
  stripe.value.card_element = stripe.value.load_stripe.elements().create("card", {
    classes: {
